...
const Rooms = () => {
  ...
  const matchesPriceRange = (rooms) => {
    return (
      selectedFilters.priceRange.length === 0 ||
      selectedFilters.priceRange.some((range) => {
        const [min, max] = range.split(" to ").map(Number);
        return rooms.pricePerNight >= min && rooms.pricePerNight <= max;
      })
    );
  };
  const sortRooms = (a, b) => {
      ...
      <div>
        <div className="flex flex-col items-start text-left">
          ...
          <p className="text-sm md:text-base text-gray-500/90 mt-2 max-w-174">
            Step into a world of refined elegance, where every detail in our
            curated collection of rooms is meticulously crafted for an
            unforgettable stay.
          </p>
        </div>
      ...
      >
        <div
          className={`flex items-center justify-between px-5 py-2.5
                min-lg:border-b border-gray-300 ${filter && "border-b"}`}
        >
          <p className="text-base font-medium text-gray-800">FILTERS</p>
          <div className="text-xs cursor-pointer">
            <span onClick={() => setFilter(!filter)} className="lg:hidden">
              {filter ? "HIDE" : "SHOW"}
            </span>
        ...
        <div
          className={`${filter ? "h-auto" : "h-0 lg:h-auto"} overflow-hidden
