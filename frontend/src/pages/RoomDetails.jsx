...
const RoomDetails = () => {
  ...
  useEffect(() => {
    if (rooms?.length) {
      ...
      setMainImage(foundRoom?.images?.[0] || assets.placeholderImage);
    }
  }, [rooms, id]);

  if (!room) {
    return <p className="text-center mt-20 text-lg">Loading room details...</p>;
  }
    ...
    <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="flex flex-col items-start gap-4 mt-10">
        <div className="flex gap-4">
          <img
            src={room?.hotel?.owner?.image || assets.placeholderImage}
            alt="Host"
            className="h-14 w-14 md:h-18 md:w-18 rounded-full"
          />
          <div>
            <p className="text-lg md:text-xl">
              Hosted by {room?.hotel?.name || "Host"}
            </p>
